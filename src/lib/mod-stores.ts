...
function hitFromNexus(m: NexusModJson): SearchHit {
  ...
  return {
    ...
    description: stripHtml(m.summary || m.description || "Nexus Mods Palworld pack."),
    body: stripHtml(m.description || m.summary || "").slice(0, 8000),
  ...
  return (json.response?.publishedfiledetails ?? [])
    ...
    .map((d) => {
      ...
      return {
        ...
        body: stripHtml(d.file_description || "").slice(0, 8000),
        kind: kind === "ini" || kind === "framework" ? "pak" : kind,
